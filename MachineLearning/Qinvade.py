...
Human = Rect(100, 100, 10, 10)
Demon = Rect(400, 400, 10, 10)
...
class DH:
    def __init__(self, step=False, image=False):
        ...
        pygame.init()
        self.screen = pygame.display.set_mode(Human.size, pygame.DOUBLEBUF)
        pygame.display.set_caption("DH Part4")
        # 素材のロード
        # self.load_images()
        # self.load_sounds()
        # ゲームオブジェクトを初期化
    ...
    def _key_action(self, action): # キー入力を代替
        ...
        if action == 1:
            ...
            self.player.rect.clamp_ip(Demon)
        # 右移動
        elif action == 2:
            self.player.rect.move_ip(self.player.speed, 0)
            self.player.rect.clamp_ip(Demon)
        ...
        if self.game_state == PLAY:
            ...
            for alien in self.aliens:
                if (alien.rect.center[0] < 10 and alien.speed < 0) or \
                        (alien.rect.center[0] > Human.width-10 and alien.speed > 0):
                    turn_flag = True
                    break
            if turn_flag:
                for alien in self.aliens:
    ...
    def draw(self, screen):
        ...
        if self.game_state == START:  # スタート画面
            ...
            title = title_font.render("DH GAME", False, (255,0,0))
            screen.blit(title, ((Human.width-title.get_width())//2, 100))
            # エイリアンを描画
            alien_image = Alien.images[0]
            screen.blit(alien_image, ((Human.width-alien_image.get_width())//2, 200))
            # PUSH STARTを描画
            push_font = pygame.font.SysFont(None, 40)
            push_space = push_font.render("PUSH SPACE KEY", False, (255,255,255))
            screen.blit(push_space, ((Human.width-push_space.get_width())//2, 300))
            # クレジットを描画
            credit_font = pygame.font.SysFont(None, 20)
            credit = credit_font.render("2019 http://pygame.skr.jp", False, (255,255,255))
            screen.blit(credit, ((Human.width-credit.get_width())//2, 380))
            ...
            screen.blit(stat, ((Human.width - stat.get_width()) // 2, 10))
            # 壁の耐久力描画
            ...
            screen.blit(gameover, ((Human.width-gameover.get_width())//2, 100))
            # エイリアンを描画
            alien_image = Alien.images[0]
            screen.blit(alien_image, ((Human.width-alien_image.get_width())//2, 200))
            # PUSH SPACEを描画
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect.center = (Human.width//2, Human.bottom - 9)
        self.reload_timer = 0
    def update(self):
        # 押されているキーをチェック
        ...
        if pressed_keys[K_LEFT]:
            ...
            self.rect.move_ip(self.speed, 0)
        #self.rect.clamp_ip(Human)
        self.rect.clamp_ip(Demon)
        # ミサイルの発射
...
class UFO(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, wave):
        ...
        self.image = self.images[0]
        self.rect = self.image.get_rect()
        self.rect.center = (Human.width - pos[0] if side else pos[0], pos[1]) # 開始位置(x)
        self.pos_kill = pos[0] if side else Human.width - pos[0] # 消滅位置(x)
    def update(self):
        # 横方向への移動
        self.rect.move_ip(self.speed, 0)
        # 指定位置まで来たら消滅
...
class Beam(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.bottom > Human.height:  # 下端に達したら除去
            self.kill()
